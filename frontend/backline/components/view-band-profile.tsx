    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const following = await followService.checkFollowing(followerType, 'BAND', id, followerBandId, followerVenueId);
                setIsFollowing(following);
            } catch (e) {
                console.error('ViewBandProfile load error:', e);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleRsvp = async (show: Show) => {
        ...
        try {
            ...
            const [fresh, freshPast] = await Promise.all([
                getShowsByProfile('band', id),
                getShowsByProfile('band', id, true),
            ]);
            setShows(fresh);
